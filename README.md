...
本系统基于 **中文放射报告 NER（命名实体识别）+ 解剖部位知识图谱 + 规则引擎**，对影像报告正文做自动化质量控制，覆盖 **性别矛盾、左右混淆、评分缺失、单位错误、描述-结论矛盾、部位不符、内部矛盾、同音错别字、自定义互斥冲突、模板合规，以及信息框-正文矛盾、前后文/上下文逻辑错误、逐部位精确比对（描述↔结论同部位正常/异常矛盾）** 等（规则族 R1–R12、R14、R15、R17、R18；R13 预留，R16 随访时限为可选项）。
...
| **R7**  | 描述内部矛盾      | 中        | 同一描述段内出现男女专属器官混用（不可能同时为真）                     |
...
| **R11** | 信息框-正文矛盾    | 高 / 中    | 患者信息框（性别/侧别/异常征）与报告正文不一致（R11-GENDER / R11-SIDE；描述异常-结论正常的段级兜底已并入 R17-NORMALITY） |
| **R12** | 句子前后文       | 中        | 单句内前后逻辑冲突（如先否定后肯定）                            |
| **R14** | 前后文（跨段）     | 中        | 检查所见与诊断印象之间计数/性质/左右矛盾（R14-COUNT / R14-NATURE / R14-SIDE；正常性矛盾已并入 R17-NORMALITY） |
| **R15** | 上下文（段内）     | 中        | 同段内正常表述/存在性/左右侧逻辑冲突（R15-NORMAL / R15-PRESENCE / R15-SIDE） |
| **R17** | 逐部位精确比对 + 正常性矛盾 | 高        | 按 `(器官, 侧别)` 精确到同一部位比对「检查所见」与「诊断印象」的正常/异常声明：**描述某部位正常、结论却写该部位病灶（或反之）即报矛盾**；左小脑正常 + 右小脑软化灶（不同侧）不误报，左小脑正常 + 左小脑软化灶（同侧）报矛盾。所有「描述↔结论正常性矛盾」（逐部位 / 段级兜底）统一归并为 **`R17-NORMALITY`** |
| **R18** | 检查部位器官漏写   | 中        | 登记部位声明了某区域（支持「胸部、上腹部」多区域拆分），但影像描述段未描述该区域任一器官（如「登记上腹部」却只写肺）→ 报漏写；整段「未见异常」整体声明不误报；多区域分别校验、互不牵连 |

> **规则按知识图谱「实体 × 关系」归类**：上述 R1–R18 平铺编号在内部已挂到解剖知识图谱的维度上，自然聚为若干**关系族**（详见 `docs/质控规则知识图谱归类.md`）：
> - `hasGender` 族：R1-GENDER / R7-INTERNAL / R11-GENDER / R12(男女混用)
> - `hasLaterality` 族：R2 / R11-SIDE / R14-SIDE / R15-SIDE（图谱 `side_ok` 决定哪些器官参与左右比对）
> - **`hasNormality` 族**：**R17-NORMALITY**（由 R11-ABNORMAL / R14-NORMAL / R17-PERREGION 合并而来，统一为单一种类，保留逐部位/段级兜底多粒度判定）
> - `consistentWith` 族：R5 / R14-NATURE / R14-COUNT / R17(逐部位)
> - `present` 族：R6 / R10 / R18
> - `hasQuality` 族：R3 / R4 / R8 / R9
> - `hasTemporal` 族：R16
>
> 前端看板可按 `kg_relation` 维度分组/筛选，而非依赖易碎的 `rule_id` 前缀解析（元数据层为后续增量，当前以 `rule_id` 稳定主键 + 关系族注释为准）。
