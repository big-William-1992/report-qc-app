...
> 面向放射科医生的报告智能质控助手：粘贴即查、复制即控、自动修正、样本沉淀、科室报表。

---

## 一、系统简介

本系统基于 **中文放射报告 NER（命名实体识别）+ 解剖部位知识图谱 + 规则引擎**，对影像报告正文做自动化质量控制，覆盖 **性别矛盾、左右混淆、评分缺失、单位错误、描述-结论矛盾、部位不符、内部矛盾、同音错别字、自定义互斥冲突、模板合规** 十大类问题（R1–R10）。

核心能力：

- **三步即用**：粘贴报告 → 一键质控 → 红色高亮定位问题
...
> ⚠️ **macOS 剪贴板权限**：首次使用「监听剪贴板」功能时，若系统弹出「Terminal / Python 请求访问剪贴板」，请允许。否则监听读不到复制内容。权限可在「系统设置 → 隐私与安全性 → 辅助功能 / 自动化」中补开。
...
| 质控结果区  | 列出每条问题（规则号 + 类型 + 严重度 + 描述 + 原文片段），并在正文中用红/黄/蓝高亮       |
...
- **🔔 发现问题即弹窗提醒**：监听命中问题时弹出醒目提醒窗（默认可用，可关）
...
| **R2**  | 左右侧混淆   | 高     | 同一解剖部位族在「检查所见」与「诊断印象」段方位（左/右）互斥           |
...
**严重度权重**：高 = −30，中 = −15，低 = −5（用于评分扣分）。

### 元信息自动抽取

监听或导入场景会自动从正文抽取 `{性别, 年龄, 检查部位, 申请部位, 侧别}`，回填输入框供人工校正。抽取仅作提示，不强制覆盖手动输入。
...
- 所有报告、样本、配置**仅存于本机**（开发态 `assets/`，打包态 `%APPDATA%/MedicalReportQC/` 或 macOS 用户目录），**不发起任何网络上传**
...
- 用「🔗 RIS 直连」页从院内数据库批量拉取并质控入库；或逐份粘贴/导入
...
```
src/
├── app.py          # Tkinter GUI：三页签 + 弹窗 + 监听循环 + 导出
├── engine.py       # 规则引擎：NER + 知识图谱 + R1–R10 + 评分 + 元信息抽取
...
- **零第三方依赖**：纯 Python 标准库（re / json / sqlite3 / difflib / tkinter）
...
- **v2.0**（当前）：R1–R10 规则、剪贴 板监听自愈合、自动修正预览、脱敏样本库、捕获审计、报表导出、命中弹窗、误报闭环、评分透明、监听去重、模板校验

适用场景：放射科日常报告质控、语音录入误写拦截、科室月度 QA 报表、规培教学样本沉淀。
