...
> 规则编号一览（持续扩充）：R1 性别矛盾、R2 左右混淆、R3 评分标准缺失、R4 单位非法、R5 描述-结论矛盾、R6 登记部位不符、R7 段内自相矛盾、R8 错别字、R9 显性冲突、R10 模板结构、R11 上下文（描述-结论/性别-正文）、R12 同一句逻辑错误、R14 前后文逻辑（描述↔结论：正常/定性/数量/左右）、R15 上下文逻辑（描述段内跨句：先正常后阳性/左右矛盾/先见后无）。
